...
def findchipsscraper(partnumber, quantity_required, search_id):
    ...
    for distributor in distributors:
        distributor_name = distributor.find('h3', class_='distributor-title').text.replace(' ', '').replace('\n', '').\
            ...
            replace('Manufacturer Direct – Inventory Available for Immediate and Future Delivery', '').replace(
            "ECIA(NEDA)", ''
        )
        for listing in distributor.find_all('tr', class_='row'):
            ...
            if string_checker(numeric_string) != 0 and string_checker(numeric_string) >= quantity_required:
