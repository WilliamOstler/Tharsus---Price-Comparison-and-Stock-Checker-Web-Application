...
#superuser_draw_key = superuser.draw_key
...
def admin():
    return render_template('admin.html', name=superuser.firstname)
...
def create_winning_draw():
    ...
    new_winning_draw = Draw(user_id=0, draw=submitted_draw, win=True, round=round, draw_key=superuser_draw_key)
...
def view_winning_draw():
    ...
    winning_draw_copy = current_winning_draw

    # if a winning draw exists
    if current_winning_draw:
        winning_draw_copy.view_decrypted_draws(superuser_draw_key)
        # re-render admin page with current winning draw and lottery round
        return render_template('admin.html', winning_draw=winning_draw_copy, name=superuser.firstname)
...
def run_lottery():
    ...
    if current_winning_draw:
        ...
        saved_winning_draw = current_winning_draw.draw
        ...
        if user_draws:
            ...
            current_winning_draw.view_decrypted_draws(superuser_draw_key)
            ...
            for draw in user_draws:
                ...
                saved_draw = draw.draw

                # decrypt the current user draw
                draw.view_decrypted_draws(user.draw_key)

                # if user draw matches current unplayed winning draw
                if draw.draw == current_winning_draw.draw:
                    current_winning_draw.draw = saved_winning_draw
                    draw.win = True
                ...
                # re-assign encrypted draw values
                draw.draw = saved_draw
                current_winning_draw.draw = saved_winning_draw
            ...
            return render_template('admin.html', results=results, name=user.firstname)
...
def logs():
    ...
    return render_template('admin.html', logs=content, name=superuser.firstname)
